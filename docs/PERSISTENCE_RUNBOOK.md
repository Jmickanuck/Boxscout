...
## Plan 009 additive catalogue updates

`node --env-file=.env.database.local scripts/database/cli.ts append` requires `BOXSCOUT_BASELINE` (the reviewed current digest) and `BOXSCOUT_REVIEWED_BY`. It takes the same transaction lock as migrations/import/publication and refuses any change/removal/reordering of existing flattened rows. Only new rows are inserted; conflicts roll back the whole transaction. Exact reruns are unchanged. The original bootstrap import guard is unchanged. Back up before/after, then parity → approve → publish as above. The fixture source now composes the immutable pilot with `data/imports/mega-expansion/reviewed.json`; unapproved input is never imported.

Migrations 005/006 permit non-base identities without a default variant, while requiring at least one collectible edition and preserving mandatory Base defaults, uniqueness, provenance and approval gates. No database request or credential is added to the browser.
